// ABOUTME: Main executable for solving chemistry ODE using header-only
// integrators (VODE) ABOUTME: Auto-generated by JAFF for chemical reaction
// network integration
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      std::cout << "Usage: chemistry_ode [t_end] [ic_file] [out_file] "
                   "[--analytic-jac|--numerical-jac]\n";
      return 0;
  ...
  std::cout << "Number of reactions: " << std::cout
            << "Number of reactions: " << $nspec$ << "\n";
  ...
  if (!ic_path.empty()) {
    ...
    if (ic_file.is_open()) {
      for (int i = 0; i < mySys.neqs && (ic_file >> y0[static_cast<size_t>(i)]);
           ++i) {
      }
      ic_file.close();
  ...
  for (int i = 0; i < mySys.neqs; ++i) {
    std::cout << "  nden[" << i << "] = " << std::setprecision(10)
  ...
  state.t = t_start;
  state.tout = t_end;
  for (size_t i = 0; i < static_cast<size_t>(ChemistryODE::neqs); ++i)
    state.y[i] = y0[i];
  state.rtol = 1.e-5;
  ...
  std::cout << "Jacobian: "
            << (state.jacobian_analytic ? "analytic" : "numerical") << "\n";
...
#define JAFF_HAS_CONSERVERATION_METADATA 1
  // $JAFF SUB nelem
  constexpr int n_elements = $nelem$;
  // $JAFF END
  ...
  if constexpr (JAFF_HAS_CONSERVATION_METADATA) {
    ...
    for (int e = 0; e < n_elements; ++e) {
      ...
      for (int j = 0; j < ChemistryODE::neqs; ++j) {
        init += static_cast<long double>(elem_matrix[e][j]) *
                static_cast<long double>(y0[static_cast<size_t>(j)]);
        fin += static_cast<long double>(elem_matrix[e][j]) *
               static_cast<long double>(state.y[static_cast<size_t>(j)]);
      }
    ...
    for (int j = 0; j < ChemistryODE::neqs; ++j) {
      q_init += static_cast<long double>(species_charge[j]) *
                static_cast<long double>(y0[static_cast<size_t>(j)]);
      q_fin += static_cast<long double>(species_charge[j]) *
               static_cast<long double>(state.y[static_cast<size_t>(j)]);
    }
    const long double q_rel =
        (q_init != 0.0L) ? (q_fin - q_init) / q_init : 0.0L;
    std::cout << "  net_charge: init=" << static_cast<double>(q_init)
              << ", final=" << static_cast<double>(q_fin)
              << ", rel_delta=" << static_cast<double>(q_rel) << "\n";
  } else {
    std::cout << "\n(Element/charge conservation metadata not available in "
                 "this build)\n";
  }

  if (result != IntegratorResult::SUCCESS) {
    std::cerr << "\nIntegration did not succeed. Error code = "
              << static_cast<int>(result) << "\n";
    // Diagnostics: print failed step index and key integrator state
    auto dump_state =
        [](const VODEState<static_cast<size_type>(ChemistryODE::neqs)> &s) {
          std::cerr.setf(std::ios::scientific, std::ios::floatfield);
          std::cerr.precision(17);
          std::cerr << "Failure diagnostics:" << '\n';
          std::cerr << "  failed_at_step = " << (s.n_step + 1)
                    << " (completed=" << s.n_step << ")" << '\n';
          std::cerr << "  t = " << s.t << ", tn = " << s.tn
                    << ", tout = " << s.tout << '\n';
          std::cerr << "  H = " << s.H << ", NQ = " << int(s.NQ)
                    << ", L = " << int(s.L) << '\n';
          std::cerr << "  ETA = " << s.ETA << ", RC = " << s.RC
                    << ", ETAMAX = " << s.ETAMAX << '\n';
          std::cerr << "  err_fails = " << s.err_fails
                    << ", n_rhs = " << s.n_rhs << ", n_jac = " << s.n_jac
                    << '\n';
          std::cerr << "  flags: ICF=" << int(s.ICF) << ", IPUP=" << int(s.IPUP)
                    << ", JCUR=" << int(s.JCUR) << ", NEWH=" << int(s.NEWH)
                    << ", NEWQ=" << int(s.NEWQ) << ", NQWAIT=" << int(s.NQWAIT)
                    << '\n';
