...
class Network:
    ...
    def __init__(self, fname, errors=False, label=None,
        ...
        # Optional symbolic terms for the internal energy RHS (d e_int / dt)
        # Users can add SymPy expressions or strings parsable by SymPy.
        # These will be included in codegen and analytically differentiated
        # as part of the Jacobian generation.
        self.energy_rhs_terms = []
    ...
    # ****************
    def add_internal_energy_term(self, expr):
        """
        Add a term to the internal energy RHS (cooling/heating), d e_int / dt.

        Parameters
        ----------
        expr : sympy expression or str
            A SymPy expression, or a string parsable by SymPy, that may depend on:
            - nden[i] (species number densities via IndexedBase 'nden')
            - eint (internal energy density symbol)
            - tgas (gas temperature symbol)
            - any auxiliary variables/functions supported by this network

        Notes
        -----
        The expression is treated like autogenerated ODE terms and will be
    ...
    # ****************
    def clear_internal_energy_terms(self):
        """Remove any previously-added internal energy RHS terms."""
        self.energy_rhs_terms = []
    ...
    # *****************
    def get_symbolic_ode_and_jacobian(self, idx_offset=0, use_cse=True, language="c++", energy_rhs_terms=None):
        """
        ...
        # Create symbolic variables representing state for Jacobian (species + energy)
        ...
        n_species = len(self.species)
        n_vars = n_species + 1  # +1 for internal energy
        y_syms = [symbols(f'y_{i}') for i in range(n_vars)]

        # Build mapping to replace any Indexed occurrences of nden[...] in expressions
        # with the corresponding scalar y_i symbols (species and energy).
        ...
        ode_symbols = [sp.Integer(0) for _ in range(n_vars)]
        for i, rea in enumerate(self.reactions):
        ...
        # Build internal energy RHS from user-specified terms, if any
        if energy_rhs_terms is None:
            energy_rhs_terms = self.energy_rhs_terms
        if energy_rhs_terms:
            # Accept either list/tuple of terms or a single expression
            if not isinstance(energy_rhs_terms, (list, tuple)):
                energy_rhs_terms = [energy_rhs_terms]
            # Sum all terms; support 'eint' and nden[...] symbols
            eint_sym = symbols('eint')
            e_rhs = sp.Integer(0)
        ...
        # Assign internal energy RHS to the last state entry
        ode_symbols[n_vars-1] += e_rhs
        ...
        if use_cse:
            ...
            ode_reduced = reduced_exprs[:n_vars]
            jac_reduced = reduced_exprs[n_vars:]
