...
def test_cse_generates_common_subexpressions(test_network):
    ...
    assert common_subexpr_count > 0, "CSE should identify common subexpressions in the test network"
...
def test_cse_reduces_redundancy(test_network):
    ...
    assert exp_count_with_cse <= exp_count_no_cse, "CSE should reduce redundant exp() calls"
...
def test_cse_output_structure(test_network):
    ...
    assert "const double" in rates_with_cse, "CSE should generate const double declarations"
    ...
    for line in lines:
        if line.strip() and not line.strip().startswith('//'):
            assert line.strip().endswith(';'), f"C++ statements should end with semicolon: {line}"
...
def test_cse_vs_no_cse_consistency(test_network):
    ...
    assert rate_count_no_cse == rate_count_with_cse, "Both versions should generate the same number of rates"
...
def test_cse_common_patterns(test_network):
    ...
    has_temp_dependent_cse = any("tgas" in line and "const double x" in line for line in lines)
    assert has_temp_dependent_cse, "CSE should extract temperature-dependent common subexpressions"
...
def test_network_reactions_loaded(test_network):
    ...
    assert len(test_network.reactions) == 8, "Test network should contain exactly 8 reactions"